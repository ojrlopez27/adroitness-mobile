package edu.cmu.adroitness.client.services.gmail.control;
...
public class ViewHelper {
    ...
    public void initializeGmail(){
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_INITIALIZE));
    }
     ...
     * Result: Capture the result as List<GmailMessageVO> object in event
     * when event.getNotification is GMAIL_AFTER_UPDATE_LABEL_LIST from event.getLabels()
     */
    public void updateLabelListFromGmail()
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_UPDATE_LABEL_LIST_FROM_GMAIL));
    }
     ...
     * Result: Capture the result as List<GmailMessageVO> object in event
     * when event.getNotification is GMAIL_AFTER_FILTER_QUERY from event.getEmails()
     */
    public void filterMessagesByQuery(GmailFilterQueryInputVO gmailFilterQueryInputVO, Boolean isSendEvent)
    {
        ...
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_FILTER_MESSAGES_BY_QUERY)
    ...
    public void getDataFromGmail() {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_DATA_FROM_GMAIL));
    }
    public void getMessage(String messageID)
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_MESSAGE)
     ...
     *  * Result: Capture the result as MimeMessage object in event
     *  from event.getEmail()
     */
    public void getMimeMessage(String messageID)
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_MIME_MESSAGE)
        .put(Constants.GMAIL_MESSAGE_ID, messageID));
    }
     ...
     * @param messageID
     * Result: Capture the result as GmailMessageVO object in event
     * when event.getNotification is GMAIL_GET_MESSAGE_CONTENT from event.getGmailMessageVO()
     */
    public void getMessageContent(String messageID, Boolean isActivityRequest)
    {
        isActivityRequest = true;
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_MESSAGE_CONTENT)
                .put(Constants.GMAIL_MESSAGE_ID, messageID)
     ...
     *  Result: Capture the result as List<GmailMessageVO) object in event
     * when event.getNotification is GMAIL_GET_MESSAGE_VO from event.getGmailMessageVO
     */
    public void getJsonEmailMessageDetails(String messageID, Boolean isListViewItemClick)
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_JSON_EMAIL_MESSAGE_DETAILS)
                .put(Constants.GMAIL_MESSAGE_ID, messageID)
     ...
     * Result: Capture the result as List<GmailMessageVO) object in event
     * when event.getNotification is GMAIL_AFTER_SEND_NEW_EMAIL from event.getNewEmails
     */
    public void sendMessage(GmailMessageVO gmailMessageVO)
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_SEND_EMAIL_MESSAGE)
    ...
    public void sendMessageWithAttachment( GmailMessageVO gmailMessageVO)
    {
        if(gmailMessageVO.getAttachedFile()!=null) {
            mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_SEND_EMAIL_MESSAGE_WITH_ATTACHMENT)
                    .put(Constants.GMAIL_MESSAGE_VO, gmailMessageVO));
        }
    }
     ...
     * Result: Capture the result as List<GmailMessageVO) object in event
     * when event.getNotification is GMAIL_AFTER_REPLY_EMAIL from event.getSendEmails
     */
    public void replyToEmail(GmailMessageVO gmailMessageVO)
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_REPLY)
                .put(Constants.GMAIL_MESSAGE_VO, gmailMessageVO));
    }

    /***
     * Get Label instance for label name in Gmail.
     * Label has properties such as getMessagesTotal, getUnreadMessagesTotal, getThreads, getThreadsUnread
     * @param labelName
     * Result: Capture the result as Label object in event
     * when event.getNotification is GMAIL_AFTER_GET_LABEL
     */
    public void getLabel(String labelName )
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_LABEL)
                .put(Constants.GMAIL_LABEL, labelName));
    }

    /***
     * Get User Profile instance for authenticated User ID in Gmail.
     * Note: User would have already chosen google account while launching InMind and
     * this user is authenticated.
     * Profile has properties such as getMessagesTotal, getThreads, getEmailID, getHistoryId
     * Result: Capture the result as Profile object in event
     * when event.getNotification is GMAIL_AFTER_GET_USER_PROFILE
     */
    public void getUserProfile( )
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_USER_PROFILE));
    }
    ...
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_FORWARD)
                .put(Constants.GMAIL_MESSAGE_VO, gmailMessageVO));
    }
    ...
    {
        mMB.send(ViewHelper.this, MBRequest.build(Constants.MSG_GMAIL_GET_ATTACHMENTS)
                .put(Constants.GMAIL_MESSAGE_ID, messageID)
